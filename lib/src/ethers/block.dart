...
class Block extends _RawBlock<_BlockImpl> {
  const Block._(super.impl) : super._();
...
class BlockWithTransaction extends _RawBlock<_BlockWithTransactionImpl> {
  const BlockWithTransaction._(super.impl) : super._();

  /// A list of the transactions this block includes.
  List<TransactionResponse> get transactions =>
      impl.transactions.cast<_TransactionResponseImpl>().map(TransactionResponse._).toList();
}

class _RawBlock<T extends _RawBlockImpl> extends Interop<T> {
  const _RawBlock._(super.impl) : super.internal();
  ...
  DateTime get timestamp => DateTime.fromMillisecondsSinceEpoch(impl.timestamp * 1000);
