...
class ContractFactory extends Interop<_ContractFactoryImpl> {
  ...
  factory ContractFactory(dynamic interface, String bytecode, dynamic providerOrSigner) {
    assert(
    ...
    assert(
      interface is String || interface is List<String> || interface is Interface,
      'abi must be valid type',
    );

    return ContractFactory._(
      _ContractFactoryImpl(
  ...
  factory ContractFactory.fromProvider(dynamic abi, String bytecode, Provider provider) =>
  ...
  factory ContractFactory.fromSigner(dynamic abi, String bytecode, Signer signer) =>
      ContractFactory._(_ContractFactoryImpl(abi, bytecode, signer.impl));

  const ContractFactory._(super.impl) : super.internal();
  ...
  Contract attach(String address) => Contract._(_ContractImpl(address, interface, signer));
  ...
  UnsignedTransaction getDeployTransaction(dynamic args, [TransactionOverride? override]) =>
  ...
  ///
  /// If the optional overrides is specified, they can be used to override the endowment value, transaction nonce, gasLimit or gasPrice.
  Future<Contract> deploy(dynamic args, [TransactionOverride? override]) async =>
